...
from pfns4mvbo.priors import bodi_prior
...
parser.add_argument('-on', '--observation_noise', type=float, required=False, default=1.0)
...
categorical_encoder = get_categorical_encoder(config_bodi['extra_prior_kwargs_dict']['hyperparameters']['categorical_encoding'])
config_bodi['extra_prior_kwargs_dict']['hyperparameters']['categorical_encoding_function'] = categorical_encoder
...
global validation
validation = get_validation_data(config_bodi, device=device)
...
np.save(loss_curve_filename, np.zeros(config_bodi['epochs']))
...
# continue_model = torch.load('/mnt/qb/work/eggensperger/eww768/PFNs4MVBO/trained_models/pfn_bodi_6.pth')
# ret = train.train(continue_model=continue_model, epoch_callback=epoch_callback, **add_criterion(config_bodi, device=device))


ret = train.train(epoch_callback=epoch_callback, **add_criterion(config_bodi, device=device))

if ret is not None:
    model = ret[2]

    # can't save the categorical encoder with the model so we delete it
    del config_bodi['extra_prior_kwargs_dict']['hyperparameters']['categorical_encoding_function']

    # add model info
    model.info = {
        'prior': 'BODi',
        'categorical_encoding': config_bodi['extra_prior_kwargs_dict']['hyperparameters']['categorical_encoding'],
        'sample_hyperparameters': config_bodi['extra_prior_kwargs_dict']['hyperparameters']['sample_hyperparameters'],
        'observation_noise': config_bodi['extra_prior_kwargs_dict']['hyperparameters']['observation_noise'],
        'training_hyperparameters': config_bodi['extra_prior_kwargs_dict']['hyperparameters'],
        'learning_rate': config_bodi['lr'],
        'bptt': config_bodi['bptt'],
    ...
    torch.save(model, f'PFNs4MVBO/trained_models/pfn_bodi_{model_number}.pth')
else:
