...
def get_model_hps(input_space, params, model=None):
    ...
    if params.get('hp_sampling_method') == 'authentic':
        ...
        model_kwargs = {'dtype': torch.float32,
                        'device': 'cpu'}
        model = BoBuilder.get_model(input_space, 'gp_hed', **model_kwargs)
        model.fit(x, y)

        # adjust noise
        if isinstance(params['observation_noise'], float):
            noise = params['observation_noise'] * model.likelihood.noise
            raw_noise = params['observation_noise'] * model.likelihood.raw_noise
        else:
            noise = model.likelihood.noise
            raw_noise = model.likelihood.raw_noise
        ...
        model_hps = [
            ...
            model.kernel.base_kernel.categorical_kernel.base_kernel.base_kernel.outputscale,
            model.kernel.base_kernel.categorical_kernel.base_kernel.base_kernel.base_kernel.lengthscale,
            model.kernel.base_kernel.categorical_kernel.base_kernel.hed_embedders,
            noise]

        if params.get('sample_raw_hps', False):
            model_hps += [
                model.kernel.raw_outputscale,
                model.kernel.base_kernel.raw_lamda,
                model.kernel.base_kernel.numeric_kernel.base_kernel.raw_lengthscale,
                model.kernel.base_kernel.numeric_kernel.raw_outputscale,
                model.kernel.base_kernel.categorical_kernel.raw_outputscale,
                model.kernel.base_kernel.categorical_kernel.base_kernel.base_kernel.raw_outputscale,
                model.kernel.base_kernel.categorical_kernel.base_kernel.base_kernel.base_kernel.raw_lengthscale,
                raw_noise
            ]
...
def get_model(input_space, x, device='cpu', **hps):
    ...
    if hps.get('sample_hyperparameters', False) and len(input_space.df_col_names) > 1:
        ...
        model.kernel.base_kernel.numeric_kernel.outputscale = model_hps[3]
        model.kernel.base_kernel.categorical_kernel.outputscale = model_hps[4]
        model.kernel.base_kernel.categorical_kernel.base_kernel.base_kernel.outputscale = model_hps[5]
        model.kernel.base_kernel.categorical_kernel.base_kernel.base_kernel.base_kernel.lengthscale = model_hps[6]
        model.kernel.base_kernel.categorical_kernel.base_kernel.hed_embedders = model_hps[7]
        model.likelihood.noise = model_hps[8]
        if False and hps.get('sample_raw_hps', False):
            model.kernel.raw_outputscale = model_hps[9]
            model.kernel.base_kernel.raw_lamda = model_hps[10]
            model.kernel.base_kernel.numeric_kernel.base_kernel.raw_lengthscale = model_hps[11]
            model.kernel.base_kernel.numeric_kernel.raw_outputscale = model_hps[12]
            model.kernel.base_kernel.categorical_kernel.raw_outputscale = model_hps[13]
            model.kernel.base_kernel.categorical_kernel.base_kernel.base_kernel.raw_outputscale = model_hps[14]
            model.kernel.base_kernel.categorical_kernel.base_kernel.base_kernel.base_kernel.raw_lengthscale = model_hps[15]
            model.likelihood.raw_noise = model_hps[16]
