...
from pfns4mvbo.priors import casmo_prior
...
parser.add_argument('-bs', '--bucket_scaling', type=float, required=False, default=1.0)
...
categorical_encoder = get_categorical_encoder(config_casmo['extra_prior_kwargs_dict']['hyperparameters']['categorical_encoding'])
config_casmo['extra_prior_kwargs_dict']['hyperparameters']['categorical_encoding_function'] = categorical_encoder
...
def add_criterion(config, device='cuda:0'):
    return {**config, 'criterion': bar_distribution.FullSupportBarDistribution(
        args.bucket_scaling * bar_distribution.get_bucket_limits(1000, ys=get_ys(config,device).cpu())
    )}


global validation
validation = get_validation_data(config_casmo, device=device)
...
np.save(loss_curve_filename, np.zeros(config_casmo['epochs']))
...
ret = train.train(epoch_callback=epoch_callback, **add_criterion(config_casmo,device=device))

if ret is not None:
    model = ret[2]

    # can't save the categorical encoder with the model so we delete it
    del config_casmo['extra_prior_kwargs_dict']['hyperparameters']['categorical_encoding_function']

    # add model info
    model.info = {
        'prior': 'CASMO',
        'categorical_encoding': config_casmo['extra_prior_kwargs_dict']['hyperparameters']['categorical_encoding'],
        'sample_hyperparameters': config_casmo['extra_prior_kwargs_dict']['hyperparameters']['sample_hyperparameters'],
        'observation_noise': config_casmo['extra_prior_kwargs_dict']['hyperparameters']['observation_noise'],
        'training_hyperparameters': config_casmo['extra_prior_kwargs_dict']['hyperparameters'],
        'learning_rate': config_casmo['lr'],
        'bptt': config_casmo['bptt'],
    ...
    for filename in filenames:
        filename_prefix = 'pfn_casmopolitan_'
    ...
    torch.save(model, f'PFNs4MVBO/trained_models/pfn_casmopolitan_{model_number}.pth')
else:
